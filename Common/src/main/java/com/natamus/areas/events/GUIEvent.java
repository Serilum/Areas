...
public class GUIEvent {
	public static void renderOverlay(GuiGraphicsExtractor guiGraphics, DeltaTracker deltaTracker) {
		if (!GUIVariables.hudMessage.equals("")) {
			...
			int colour = ARGB.color(GUIVariables.guiOpacity, ConfigHandler.HUD_RGB_R, ConfigHandler.HUD_RGB_G, ConfigHandler.HUD_RGB_B);
			if (!GUIVariables.rgb.equals("")) {
				...
				if (rgbs.length == 3) {
					try {
						...
						colour = ARGB.color(GUIVariables.guiOpacity, r, g, b);
					}
			...
			guiGraphics.text(font, GUIVariables.hudMessage, (int)(Math.round((width / 2F) / modifier) - stringWidth/2), ConfigHandler.HUDMessageHeightOffset, colour, ConfigHandler.showHUDTextShadow);
